...
import torch
import numpy as np
def print_topk_categories(k, categories, probs=None):
    if  probs is None:
        return 'Please pass the probabilities as the argument'
    else:
...
def visualize_img(attrs, inp, default_cmap, titles):
    return visualize_image_attr_multiple(np.transpose(attrs.squeeze().cpu().detach().numpy(), (1,2,0)),
